...
> Last updated: 2026-07-30 — ✅ **סקשן צוות התכנון חי באוויר** (deploy `a4fa5e4`): מולכו אדריכלים (1978) + סטודיו ליר סטוצקי (2018), ממוסגר כסיפור של מפגש בין ותק לחידוש. journal: `~/.claude/skills/session-journal/logs/2026-07-30-next60-design-team.md`.

## ✅ סקשן צוות התכנון (2026-07-30) — deploy `a4fa5e4`, חי ומאומת על next60.co.il
- **הבקשה:** להוסיף לאתר החי את זה שמולכו עושה את האדריכלות וליר סטוצקי את עיצוב הפנים — טקסט + תמונות, he+en, בלי לשבור כלום.
- **מקור החומר:** `Downloads\מצגת אמבסי אחרונה להפצה.pdf` (45MB, 63 עמ'). עמ' 61 = שתי הביוגרפיות + לוגו מולכו (xref 530, 498×90 bitmap, אין וקטור). עמ' 25 = הציטוט של ליר + הפורטרט (xref 290, 725×1024). **לא** במצגת `EMBASSY COURT 17.08.pdf` הישנה.
- **החלטות שהמשתמש נתן:** (1) ההיקף = **כל מותג The Next 60**, לא רק אמבסי קורט; (2) מיקום אחרי `#about`, **בלי קישור בתפריט**; (3) איות **STOTZKY** (המקור סותר את עצמו: הכותרת STOTZKY, הביו והאימייל Stotsky — תוקן לאחידות); (4) כן לציטוט+פורטרט; (5) **הזווית השיווקית: סיפור של שילוב הישן והחדש** — מולכו=הישן, ליר=החדש. זה הציר שהסקשן נבנה סביבו (1978 מול 2018, ארבעים שנה בדיוק).
- **מה נבנה:** `#design-team` — כותרת "כשניסיון של עשורים פוגש שפה עיצובית חדשה", שני כרטיסי זכוכית (מתכון `.about-badge`) זהים במבנה, קו מפריד מרכזי + ✦ זהב (star.svg inline) שמסמן את נקודת המפגש, ורצועת ציטוט עם הפורטרט. 15 מפתחות i18n × 2 שפות (סה"כ 101 לכל מילון).
- **פתרונות עיצוביים ששווה לזכור:** (1) **בעיית האיזון** — לכרטיס A לוגו גרפי, ל-B רק טיפוגרפיה: נפתר ע"י **לוח סימן משותף** ב-`min-height` זהה (הלוח קובע את הקצב, לא התוכן) + ספרת שנה ב-Pinyon זהב בשניהם. (2) **הפורטרט על רקע לבן** — לא לעשות key ללבן (פרינג' בכל שערה); במקום זה הרקע הלבן **הופך למסגרת**: לוח `--wolf` + `mix-blend-mode: multiply` + **`isolation: isolate` חובה** (בלעדיו ה-multiply מוכפל על הירוק והתמונה נראית שחורה). (3) **הלוגו הלבן** — luminance→alpha עם gain 1.22 ורצפת <10→0; ה-gain מפצה על הדקקת קווי 1px, הרצפה מונעת הילה אפורה.
- **מלכודת שנמנעה:** `.tw-scroll` הוא **ראש-כתיבה גלובלי יחיד** (app.js) — כל האלמנטים נאספים למערך אחד וממופים מ-`els[0].top` עד `els[last].bottom`. הוספת `tw-scroll` בסקשן שנמצא 6 מסכים מתחת למניפסט הייתה מותחת את חלון הכתיבה על כל העמוד ומשאירה את המניפסט ~90% לא-כתוב. **בסקשן החדש: רק `reveal`.** אומת A/B מול baseline: המניפסט מגיע ל-100% (574/574 תווים) לפני ואחרי.
- **תוספת:** `data-i18n-alt` ב-`setLanguage` — סוגר את פער ה-alt הלא-מתורגם שהיה ידוע. עכשיו אפשר לתרגם alt של כל תמונה.
- **אימות:** he+en ב-1903/1440/981/979/390 — סדר עמודות נכון בשני הכיוונים (כרטיס A מוביל בשתיהן), הפורטרט תמיד מתחת לכרטיס B, אפס גלילה אופקית, קצוות ה-breakpoint 981/979 תקינים. רגרסיה A/B מול baseline: typewriter 100%, connector מצייר, scroll rail זהה (84.9% מול 84.0%). נגישות: ניגודיות/פונט קריא/עצירת אנימציות/160% — הכל תקין. אומת חי על next60.co.il בשתי השפות.
- **פתוח:** הלוגו של מולכו הוא bitmap 441×87 (~2x בלבד) — שווה לבקש SVG/EPS ממשרד מולכו. אין פורטרט של יעקב מולכו (רק לליר) — נפתר עיצובית ע"י הוצאת הפורטרט לרצועה נפרדת.
...
- **בוצע:** המקור (245MB, 1080p, H.264/AAC, 16Mbps) קודד מחדש web-optimized → `assets/video/embassy-court.mp4` (~60MB, H.264 high CRF23, `+faststart`, AAC 128k). **שם הקובץ נשמר** → אפס שינויי קוד (src מוזרק ב-[app.js](../../app.js#L174)). ה-poster חודש מפריים של הסרט החדש.
- **State:** הושלם, סונכרן בשני העותקים, push אומת (`c342d3a`, ahead=0). GitHub warning: הקובץ 59.6MB > 50MB recommended (עדיין מתחת ל-100MB hard limit, נדחף תקין). preload=none → אין השפעה על טעינת העמוד.
- **פתוח:** הוצע למשתמש 720p (~20-25MB) כחלופה קלה — לא נבחר. אופציה לאימות חזותי ב-modal החי לא בוצע.

## החלפת + כוונון תמונת ההירו הראשית (2026-06-25) — deploy `2257a3c`→`62771ef`
- **בקשה:** להחליף את תמונת ההירו העליונה ב-`קבצים מהמעצבת/Hero2.jpg.jpeg`, שהכותרת+כותרת-המשנה יישבו על השמיים מעל הבניין, שהבניין לא ייחתך למטה, שהתמונה תתפוס את כל רוחב המסך, תאימות he+en, בלי לשבור כלום.
- **בוצע:** ההדמיה (1440×952) אופטמה ל-`assets/figma/hero-night.jpg` (progressive, 194KB); עודכנה ההפניה היחידה ב-[index.html](../../index.html#L59) (src+alt+width/height). פריסה: עברתי מ-`object-fit:cover` (גודל בניין נקבע ע"י רוחב → חיתוך בלתי-נמנע) ל-**`height:auto` ברוחב מלא ביחס טבעי** (הבניין לעולם לא נחתך), עם `min-height: max(100svh, calc(66.1vw + 9rem))` שיוצר רצועת שמיים נייבי מעל הבניין לטקסט. נייד (`max-aspect-ratio:3/2`) נשאר cover עם `min-height:100svh`.
- **כוונון איטרטיבי (3 סבבים):** (1) swap בסיסי; (2) `118svh`+`object-position:center bottom` — עדיין חיתוך/חפיפה; (3) **full-bleed natural** פתר; (4) רצועת שמיים `15rem`→`9rem` לצמצום הרווח מתחת לכותרת-המשנה לבקשת המשתמש.
- **אימות:** headless Chrome, he(RTL)+en(LTR), 1903/1440/390 — בכל התצוגות: כותרת-משנה מעל הבניין, בניין שלם (גג+בסיס+רחוב+דשא), רוחב מלא.
- **State:** הושלם, סונכרן בשני העותקים, push אומת (`62771ef`, ahead=0).
